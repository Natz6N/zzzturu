...
const makeMessagesRecvSocket = (config) => {
    ...
    const handleGroupNotification = (participant, child, msg) => {
        ...
        switch (child === null || child === void 0 ? void 0 : child.tag) {
            ...
            case 'revoked_membership_requests':
                ...
                break;
            default:
            // console.log("Natzsixn-DEBUG:", JSON.stringify({ ...child, content: Buffer.isBuffer(child.content) ? child.content.toString() : child.content, participant }, null, 2))
        }
    };
    ...
    const handleMessage = async (node) => {
        ...
        try {
            await Promise.all([
                processingMutex.mutex(async () => {
                    var _a, _b, _c, _d, _e, _f;                                      try {
                         await decrypt();
                     } catch (err) {
                         const errStr = String(err);
              if (errStr.includes('Bad MAC') || errStr.includes('Invalid MAC')) {
                logger.warn({ msgId: node.attrs.id, from: node.attrs.from, participant: node.attrs.participant, senderPn: node.attrs.sender_pn, senderLid: node.attrs.sender_lid }, 'Bad MAC detected - requesting retry');
                             msg.messageStubType = WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT;
                             msg.messageStubParameters = [errStr];
                         } else {
                             throw err;
                         }
                     }
