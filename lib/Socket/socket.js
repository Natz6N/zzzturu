...
const makeSocket = (config) => {
    ...
    if (url.protocol === 'wss' && ((_a = authState?.creds)?.routingInfo)) {
    ...
    try {
        ...
        signalRepository = makeSignalRepository({ creds, keys });
    }
    ...
    const waitForMessage = async (msgId, timeoutMs = defaultQueryTimeoutMs) => {
        ...
        try {
            const result = await (0, Utils_1.promiseTimeout)(timeoutMs, (resolve, reject) => {
                ...
                ws.on(`${Defaults_1.DEF_TAG_PREFIX}${msgId}`, onRecv);
                ws.on('close', onErr);
                ws.off('error', onErr);
            });
            return result;
        } finally {
            ws.off(`${Defaults_1.DEF_TAG_PREFIX}${msgId}`, onRecv);
            ws.off('close', onErr);
            ws.off('error', onErr);
        }
    };
    ...
    const validateConnection = async () => {
        let helloMsg = WAProto_1.proto.HandshakeMessage.fromObject({
    ...
    const startKeepAliveRequest = () => (keepAliveReq = setInterval(() => {
        ...
        if (diff > keepAliveIntervalMs + 5000) {
            ...
            sendNode({
                tag: 'iq',
                attrs: { id: generateMessageTag(), to: WABinary_1.S_WHATSAPP_NET, type: 'get', xmlns: 'w:p' },
    ...
    ws.on('close', () => {
        if (!ws.isOpen && !authState?.creds?.me) return;
        end(new boom_1.Boom('Connection Terminated', { statusCode: Types_1.DisconnectReason.connectionClosed }));
    });
            ...
            if (node.attrs.lid && authState.creds.me?.id) {
                ...
                process.nextTick(async () => {
                    try {
                        ...
                        const decoded = (0, WABinary_1.jidDecode)(myPN);
                        if (decoded?.user) {
                            const deviceStr = (decoded.device || 0).toString();
                            await authState.keys.set({
                                'device-list': {
                                    [decoded.user]: [deviceStr]
                                }
                            });
                        }
