...
function decodeMessageNode(stanza, meId, meLid) {
    ...
    if ((0, WABinary_1.isJidUser)(from) || (0, WABinary_1.isLidUser)(from) || (0, WABinary_1.isHostedPnUser)(from) || (0, WABinary_1.isHostedLidUser)(from)) {
    ...
    const fromMe = (0, WABinary_1.isJidNewsletter)(from) ? !!((_d = stanza.attrs) === null || _d === void 0 ? void 0 : _d.is_sender) || false : (((0, WABinary_1.isLidUser)(from) || (0, WABinary_1.isHostedLidUser)(from)) ? isMeLid : isMe)(stanza.attrs.participant || stanza.attrs.from);
...
const storeMappingFromEnvelope = async (stanza, sender, repository, decryptionJid, logger) => {
    const senderAltLid = stanza?.attrs?.sender_lid;
    const senderAltPn = stanza?.attrs?.sender_pn;
    try {
        if (senderAltLid && (0, WABinary_1.isLidUser)(senderAltLid) && (0, WABinary_1.isPnUser)(sender) && decryptionJid === sender) {
            await repository?.lidMapping?.storeLIDPNMappings?.([{ lid: senderAltLid, pn: sender }]);
            await repository?.migrateSession?.(sender, senderAltLid);
        }
        else if (senderAltPn && (0, WABinary_1.isPnUser)(senderAltPn) && (0, WABinary_1.isLidUser)(sender) && decryptionJid === sender) {
            await repository?.lidMapping?.storeLIDPNMappings?.([{ lid: sender, pn: senderAltPn }]);
            await repository?.migrateSession?.(senderAltPn, sender);
        }
    }
    catch (error) {
        logger.warn({ sender, senderAltLid, senderAltPn, error }, 'Failed to store LID mapping');
    }
};
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    try {
                        ...
                        fullMessage.messageStubParameters = [String((err === null || err === void 0 ? void 0 : err.message) || err)];
                    }
                }
            }
