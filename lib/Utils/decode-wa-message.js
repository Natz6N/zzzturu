...
const decryptMessageNode = (stanza, meId, meLid, repository, logger) => {
    ...
    return {
        ...
        async decrypt() {
            ...
            const participantPn = stanza?.attrs?.participant_pn;
            const peerRecipientPn = stanza?.attrs?.peer_recipient_pn;
                        ...
                        switch (e2eType) {
                            case 'skmsg':
                                msgBuffer = await repository.decryptGroupMessage({
                                    group: sender,
                                    authorJid: (0, WABinary_1.isLidUser)(author) ? (participantPn || author) : author,
                                    msg: content
                                });
                                break;
                            case 'pkmsg':
                            case 'msg':
                                let user = (0, WABinary_1.isJidUser)(sender) ? sender : author;
                                if ((0, WABinary_1.isLidUser)(user)) {
                                    user = senderPn || peerRecipientPn || participantPn || user;
                                }
